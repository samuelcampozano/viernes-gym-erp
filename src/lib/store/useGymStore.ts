...
export const useGymStore = create<GymState>((set, get) => ({
  ...
  updateEquipmentStatus: (id, status, notes, targetZone) => {
    ...
    set((state) => {
      ...
      const operationalCount = equipment.filter((e) => e.status === 'operational' || e.status === 'in_use').length;
    ...
    if (updatedItem) {
      setTimeout(() => {
        set((state) => ({
          equipment: state.equipment.map((e) => (e.id === (updatedItem as GymEquipment).id ? { ...e, highlighted: false } : e)),
        }));
      }, 4000);
    }
  ...
  highlightEquipment: (id, durationMs = 4000) => {
    set((state) => ({
      equipment: state.equipment.map((e) =>
        e.id === id || e.name.toLowerCase().includes(id.toLowerCase()) ? { ...e, highlighted: true } : e
      ),
    }));
    setTimeout(() => {
      set((state) => ({
        equipment: state.equipment.map((e) =>
          e.id === id || e.name.toLowerCase().includes(id.toLowerCase()) ? { ...e, highlighted: false } : e
        ),
      }));
    }, durationMs);
  },
  ...
  manageClassSchedule: (action, payload) => {
    ...
    if (action === 'reschedule' && payload.classId) {
      ...
      return { success: true, message: `Rescheduled ${targetClass.title} to ${payload.timeSlot || targetClass.timeSlot}`, affectedClass: updatedClasses.find((c) => c.id === payload.classId) };
    }

    if (action === 'reassign_trainer' && payload.classId && payload.trainerId) {
      const trainer = trainers.find((t) => t.id === payload.trainerId || t.name.toLowerCase().includes(payload.trainerId!.toLowerCase()));
      ...
      set({ classes: updatedClasses });
      return { success: true, message: `Reassigned Coach ${trainer.name} to class.`, affectedClass: updatedClasses.find((c) => c.id === payload.classId) };
    }
  ...
  simulateRevenueForecast: (priceAdjustmentPercent = 0, classCapacityDelta = 0, churnReductionTargetPct = 0) => {
    const { telemetry } = get();
    const baseMRR = telemetry.mrr;

    // Financial sensitivity calculation
    const priceFactor = 1 + priceAdjustmentPercent / 100;
    const capacityRevenueBoost = classCapacityDelta * 149 * 4; // delta * avg class member value
